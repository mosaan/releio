...
- **説明**: 同じプロバイダータイプでも複数の設定を作成・管理（例：会社用OpenAI、個人用OpenAI）。各設定は独立したAPIキー、baseURL、モデルリストを持つ。デフォルト選択の編集UIは未提供で、ModelSelectorがlocalStorageに保持する。削除は無効化後のみ可能。
- **優先度**: 高
- **状態**: 実装済み（UI制約あり）
- **関連文書**: `src/backend/settings/ai-settings.ts`

### 1.1c モデルのAPI経由自動リフレッシュ ⭐
- **説明**: プロバイダーAPI（OpenAI/Azure）からモデルリストを取得し、返却があればAPIモデルを置換（カスタムモデル保持）。返却が空の場合は既存モデルを維持。`modelRefreshEnabled`は保存のみで自動実行なし。
- **優先度**: 中
- **状態**: 部分実装（手動リフレッシュのみ）
- **関連文書**: `src/backend/settings/ai-settings.ts`
...
- **優先度**: 高
- **状態**: 実装済み
- **関連文書**: `docs_UP/設計_設計モデル.md`, `docs_UP/設計_ユースケース設計.md`
