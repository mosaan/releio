...
- **モデル更新フラグ**: `modelRefreshEnabled`は設定として保持するのみで、自動更新ロジックは別途実装が必要。
- **デフォルト選択**: `defaultSelection`はAI設定レコードに保持し、現状は移行値/手動削除のみ。UIはModelSelectorでの選択をlocalStorageに保存し、直接の編集導線はない。
