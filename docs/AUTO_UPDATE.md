...
**Location**:
- Windows: `<installation-directory>/updater.json` (e.g., `C:\Users\<username>\AppData\Local\Releio\updater.json`)
...
2. **Installer files**
   - Windows: `releio-0.1.0-setup.exe`
...
update-server/
├── latest.yml
├── releio-0.1.0-setup.exe
├── releio-0.2.0-setup.exe
└── ...
```
...
This creates:
- `dist/releio-0.1.0-setup.exe` - Installer
...
aws s3 cp dist/releio-0.1.0-setup.exe s3://my-bucket/updates/
