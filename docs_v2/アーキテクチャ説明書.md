...
- 目的: v1のカスタムAI統合をMastraベースに置き換えつつ、既存の3プロセス構成と汎用基盤（IPC・ロギング・設定/DB・圧縮/MCP連携の土台）を保持したまま、最小MVPのストリーミングチャットを成立させること。
- フェーズ: 方向づけフェーズ（PoCに近いがUIから操作可能）。永続化はインメモリ、Mastra経路は既存チャット経路と隔離。
- スコープ外: Mastra経路でのAzureプロバイダー、DB永続化、Resource/Threadメモリ共有、Mastra側MCPツール呼び出し。
