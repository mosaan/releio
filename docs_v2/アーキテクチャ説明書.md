...
### 5.1 論理構造ビュー（PlantUML／実装ベースの主要コンポーネント）
...
package "Renderer" {
  class "components/MastraMvpChat.tsx" as MastraMvpChat
  class "lib/mastra-client.ts (module)" as MastraClient
  MastraMvpChat --> MastraClient : 呼び出し
}

package "Preload" {
  class "preload/server.ts" as PreloadServer
  MastraClient --> PreloadServer : IPC invoke (window.backend)
}

package "Backend" {
  class "handler.ts" as Handler
  class "mastra/MastraChatService.ts" as MastraService
  class "common/connection.ts" as Connection
  Handler --> MastraService : delegate
  Handler --> Connection : publishEvent/onEvent
}

package "Main" {
  class "main logger (electron-log main scope)" as MainLogger
}

PreloadServer --> Handler : MessagePort invoke
Handler --> MastraMvpChat : events (mastraChatChunk/End/Error)
MastraService --> MainLogger : stdout -> main log (electron-log main)
MastraMvpChat --> MainLogger : renderer log (electron-log renderer) -> file
@enduml
```

#### ログ集約の見え方
- Main/Backend: Backendの標準出力をMainが受け取り、`electron-log`（mainスコープ）で`tmp/logs/app.log`へ書き込み。Backendも`backend/logger.ts`経由で同ファイルに出力。
- Renderer: `@renderer/lib/logger`は`electron-log/renderer`を利用し、同じ`app.log`に追記される。IPC不要でファイルに直接書けるため、RendererからもMain/Backendと同一ログを参照可能。
- したがって「Main logger」は専用クラスではなく`electron-log`メインスコープであり、Rendererは自前スコープ（renderer）を通じて同一ファイルへ出力する構成。
## 6. ログとデバッグ
