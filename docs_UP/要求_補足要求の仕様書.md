...
- **実装状況**: libsql + Drizzleで`chat_sessions/chat_messages/message_parts/tool_invocations/session_snapshots`を運用。ユーザーメッセージは送信直前に保存し、アシスタントメッセージはストリーム完了時に保存、ツール結果はライフサイクル更新で`tool_result`パートを生成。ページングやメッセージ編集UIは未対応（`deleteMessagesAfter`による以降削除のみ）。
...
### 2.4 チャットエラーUX
- **US-10**: チャット送信失敗時、履歴中にエラー行を表示し、ユーザーが失敗を認識できるようにしなければならない（現状: `MessagePrimitive.Error`で簡易表示のみ）。
- **US-11**: 失敗したユーザー入力は自動で入力欄に復元し、再送を容易にしなければならない（現状: 未実装）。
- **US-12**: エラーの詳細（プロバイダー応答、ネットワーク種別など）を展開表示できるUIを提供しなければならない（現状: `aiChatError`は文字列のみで分類なし）。
...
- **実装状況**: ストリーム失敗時は`aiChatError`の文字列イベントのみ返却し、ユーザーメッセージは既にDBに残る。自動復旧や詳細分類は未実装で、再送は手動。
