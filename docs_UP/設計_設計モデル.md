...
## チャットセッション永続化（現行実装）
- **スタックと形式**: libsqlクライアント + Drizzle ORM。全タイムスタンプはUnix msで保存し、API境界でISO 8601に変換。データベースは開発`./tmp/db/app.db`/本番`userData/db/app.db`に配置。
- **テーブル構成**:  
  - `chat_sessions`: セッションメタ（title/providerConfigId/modelId/color/metadata、messageCount、lastMessageAt、pinned/archived、summary/summaryUpdatedAt、dataSchemaVersion）。  
  - `chat_messages`: role/state/sequence、token使用量、error/metadata、親メッセージ参照。  
  - `message_parts`: `kind`=`text|tool_invocation|tool_result|attachment|metadata`。`toolCallId`はuniqueで、status/errorCode/errorMessage、関連パートID、コンテンツのtext/jsonを保持。  
  - `tool_invocations`: ツール呼び出しライフサイクル（invocationPartId/resultPartId、status、latencyMs、startedAt/completedAt、input/output JSON）。  
  - `session_snapshots`: 圧縮用サマリ（kind=summary、messageCutoffId、tokenCount、contentJson）。
- **保存フロー**: Rendererの`AIRuntimeProvider`が`chatSessionId`付き送信時にユーザーメッセージを`addChatMessage`で即時保存。Backendの`ai/stream.ts`がストリーミングパートを累積し、完了時にアシスタントメッセージを`ChatSessionStore.addMessage`へ一括保存。ツール結果は`recordToolInvocationResult`で`tool_result`パートと`tool_invocations`を更新。`messageCount`でsequenceを採番し、`settings.lastSessionId`で最終セッションを保持。
- **復元/表示フロー**: `Handler.getChatSession`がメッセージとパート/ツールをJOINし、`session_snapshots`由来の`compressionSummaries`を付与。Rendererは`convertMessagesToThreadFormat`＋`insertCompressionMarkers`で`assistant-ui`ランタイムに取り込み、セッション切替時に履歴を即時再構築する。
- **制約と課題**: ページングやメッセージ編集UIは未実装（`deleteMessagesAfter`による以降削除のみ）。`aiChatError`は文字列のみで分類なし、入力欄の自動復元は未対応。
...
- AI設定V2の現行契約の明文化（`ai_v2`/`ai`キーの移行、modelRefreshEnabled未実装、API空レス時は既存モデル維持、defaultSelection UI欠如の対処方針）。
